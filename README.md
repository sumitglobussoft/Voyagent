...
v0 alpha, live at [voyagent.globusdemos.com](https://voyagent.globusdemos.com) as of 2026-04-15. End-to-end on prod: in-house auth + tenant invites + TOTP 2FA + API keys, the full chat surface (ChatGPT-style shell + sidebar + markdown + tool cards + regenerate + session titles + code-block syntax highlighting + Cmd+K command palette), enquiry lifecycle, approvals inbox with audit trail, admin-only audit log viewer with CSV export, receivables/payables/itinerary/trial-balance reports against real invoices + double-entry ledger tables, hotels search + rate-check through TBO, per-tenant agent runtime settings (model / prompt suffix / rate limits / daily token budget), cost tracking per turn, dark mode, `en`/`hi` i18n, toast notifications, mobile drawer + loading skeletons, and a demo account exposed on the sign-in page (`demo@voyagent.globusdemos.com` / `DemoPassword123!`, isolated tenant). Hotels booking, Amadeus ticket issuance, the Tally desktop bridge, VFS portal automation, real email delivery, desktop installer signing, off-site backups, and the staging environment are all wired in code + CI but blocked on external credentials / vendor onboarding — see [docs/VENDOR_ONBOARDING.md](./docs/VENDOR_ONBOARDING.md) and [docs/STAGING.md](./docs/STAGING.md) for the prerequisites. Test suites: **~1000 Python tests** (pytest), **~100 TypeScript tests** (vitest across `@voyagent/sdk`, `@voyagent/chat`, mobile OCR), **164 Playwright E2E tests** against the live deployment including open-redirect safety + accessibility sweeps via axe-core, **24 shell assertion tests** for infra scripts. Every feature has unit + functional coverage per standing rule.
...
| Shipped surfaces | Chat (+ Cmd+K), approvals inbox, enquiry lifecycle, audit log viewer (admin RBAC) + CSV export, receivables / payables / itinerary / trial-balance reports, profile + tenant settings (invites, roles, locale, model, budget) |
| UX             | Dark mode, en/hi i18n, toast notifications, mobile drawer, loading skeletons, 404/500 pages, command palette |
| Observability  | Sentry (API + web + marketing), Prometheus `/internal/metrics`, structured tenant tagging, PII-scrubbing `before_send` |
